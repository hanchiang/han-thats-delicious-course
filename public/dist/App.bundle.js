...
function loadPlaces(map) {
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    // create a bounds, to set a suitable default zoom value
    var bounds = new google.maps.LatLngBounds();
    // Show pop up when marker is clicked
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      var stores = res.data;
      if (stores.length > 0) {
        searchResults.innerHTML = _dompurify2.default.sanitize(searchResultsHTML(stores));
      } else {
        searchResults.innerHTML = _dompurify2.default.sanitize('<div class="search__result">No results for <em>' + _this.value + '</em></div>');
      }
    }).catch(function (err) {
      console.error(err);
    });
